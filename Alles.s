;16Mhz clock made some changes to the pwm with regards to timer 4 and the prescaler changed to 79
...
; ========== CORRECTED PWM SETUP ==========
; Using Timer4 for PWM

    MOVLB   0x0F                  ; Switch to Bank F for PWM registers
    
    ; Set PWM period for 50kHz with 16MHz oscillator
    ; PR4 = ((Period x Fosc) / (4 * TMR4 Prescaler)) - 1
    ; = ((20e-6 * 16MHz) / (4 * 1)) - 1 = 79
    movlw   79                    ; Changed from 19 to 79 for 16MHz
    movwf   PR4
    
    ; Initial duty cycles (start with 0)
    movlw   0
    movwf   CCPR1L
    movwf   CCPR2L
    ...
    ; CCP1TSEL = 01 (bits 1:0) for Timer4
    ; CCP2TSEL = 01 (bits 3:2) for Timer4
    movlw   0b00000101           ; CCP2TSEL=01, CCP1TSEL=01
    movwf   CCPTMRS0
    ...
    ; Setup CCP pins as outputs (RC2 = CCP1, RC1 = CCP2)
    bcf     TRISC, 2             ; RC2 = CCP1 output (Motor A PWM)
    bcf     TRISC, 1             ; RC1 = CCP2 output (Motor B PWM)
    
    ; Setup RB6 and RB7 as outputs for motor direction control
...
GO_Straight:
    ...
    bsf     LATB,6               ; Motor A forward
    bsf     LATB,7               ; Motor B forward
    
    ; For 25% duty cycle with PR4=79: 25% of 80 = 20
    movlw   74                   ; 25% duty cycle
    movwf   CCPR1L              ; Motor A speed
    movwf   CCPR2L              ; Motor B speed
    
    ; Clear 2 LSBs for both (duty cycle bits 1:0 = 00)
    bcf     CCP1CON,4
    bcf     CCP1CON,5
    bcf     CCP2CON,4
    bcf     CCP2CON,5
...
Stop_Car:
    ...
    movlw   0
    movwf   CCPR1L              ; Motor A off
    movwf   CCPR2L              ; Motor B off
...
Right:
    ...
    movlw   74                   ; Left motor faster (31% duty)
    movwf   CCPR1L   
    
    movlw   74                   ; Right motor slower (19% duty)
    movwf   CCPR2L 
...
Left:
    ...
    movlw   74                   ; Left motor slower (19% duty)
    movwf   CCPR1L   
    
    movlw   74                   ; Right motor faster (31% duty)
    movwf   CCPR2L 
...
Right_Right:
    ; Sharp right turn - left forward, right reverse
    bsf     LATB,6               ; Left motor forward
    bsf     LATB,7               ; Right motor reverse
    
    movlw   74                   ; Left motor forward at good speed
    movwf   CCPR1L   
    
    movlw   74                   ; Right motor reverse at moderate speed;20
    movwf   CCPR2L 
...
Left_Left:
    ; Sharp left turn - right forward, left reverse
    bsf     LATB,6               ; Left motor reverse
    bsf     LATB,7               ; Right motor forward
    
    movlw   74                   ; Left motor reverse at moderate speed;20
    movwf   CCPR1L   
    
    movlw   74                   ; Right motor forward at good speed
    movwf   CCPR2L 
